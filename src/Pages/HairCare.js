...
import {hairCareProducts} from "../store/slices/hairCareProducts-slice";
import Footer from "../Component/Footer/Footer";
import {filterBySort} from "../store/slices/hairCareProducts-slice";
...
const HairCare = () =>{
    const {hairCareItems , status, filteredItem} = useSelector(state => state.hairCare);
    const dispatch = useDispatch();
    useEffect( () =>{
        dispatch(hairCareProducts())
    },[]);
    return(
        <React.Fragment>
                ...
                <section className={classes['productsPage__products']}>
                    <div className={classes['productsPage__filterByAvailability']}>
                        <div className={classes['productsPage__selectField']}>
                            <span>{hairCareItems.length}  :تعداد محصولات </span>
                        </div>
                    </div>
            ...
            <SuggestionProducts items={hairCareItems} status={status}/>
            <Footer/>
        </React.Fragment>
    )
}
